...
const server = http.createServer((req, res) => {
  if (req.url === "/" && (req.method === "GET" || req.method === "HEAD")) {
    // The RelayMiner's backend ping hits the root; anything but a 2xx marks the backend unreachable.
    return sendJson(res, 200, { service: SERVICE, status: "ok" });
  }
  if (req.method === "GET" && req.url === "{{VERSION_PATH}}") {
