#!/usr/bin/env bash
# One-time migration on the example host: moves the supplier stack out of the first service's
...
set -euo pipefail
OLD=/opt/pocket/services/example-builder-test
NEW=/opt/pocket/supplier
OLDPROJ=example-builder-test
...
docker compose -p deploy -f /opt/pocket/services/example-charts/deploy/docker-compose.yaml down 2>/dev/null || true
...
cd "$OLD/deploy" && docker compose -p example-builder-test-backend up -d --build 2>&1 | tail -2
cd /opt/pocket/services/example-charts/deploy && docker compose -p example-charts up -d 2>&1 | tail -2
...
echo "public endpoint: HTTP $(curl -sk -o /dev/null -w '%{http_code}' https://services.example.org/)"
sleep 20
