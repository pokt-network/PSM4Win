...
describe('validation regexes (contract 6.15)', () => {
  ...
  it('service ids and names', () => {
    expect(() => validateServiceId('pretty-charts')).not.toThrow()
    expect(() => validateServiceId('a'.repeat(43))).toThrow(SignerFailure)
    expect(() => validateServiceId('bad id')).toThrow(/Service ID/)
    expect(RE.serviceName.test('Pretty Charts')).toBe(true)
    expect(RE.serviceName.test('Bad;Name')).toBe(false)
  })
  it('wallet names', () => {
    expect(validateWalletName('app-pretty-charts')).toBe('app-pretty-charts')
  ...
  it('paths and urls', () => {
    ...
    expect(RE.backendUrl.test('http://pretty-charts-backend:8080')).toBe(true)
