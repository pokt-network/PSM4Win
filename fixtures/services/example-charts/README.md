...
| | |
|---|---|
| Service ID | `example-charts` (free on Beta and MainNet as of 2026-09-13) |
| Name | Example Charts |
...
| Deploy bundle | [`deploy/`](deploy/), for the example supplier host |
...
| `GET /v1/version` | Identity probe: `{"service":"example-charts","version":...,"engine":{...}}` |
...
```json
{
  "service": "example-charts", "format": "svg", "content_type": "image/svg+xml",
...
```bash
cd services/example-charts/backend
npm install
npm test          # starts the server with small limits and runs 30 checks
npm start         # listens on :8080
```

`docker build -t example-charts-backend .` produces the image (about 280 MB, Alpine). The Skill's `lint_backend.py --base-url http://127.0.0.1:8080 --card ../card.json` grades a running backend the way SAGE would.

## Deployment on Beta

The example host runs one supplier stack (redis, miner, relayer, Caddy at `https://services.example.org`) in `/opt/pocket/supplier`, described in [`servers/example-host/`](../../servers/example-host/). A supplier serves every service it is staked for through the same relayer, so Example Charts is only a backend container on that stack's `pocket-supplier` network plus one entry in the relayer config.

1. **Ship and start the backend** from the workstation: `bash services/example-charts/deploy/deploy.sh`. Copies `backend/` and `deploy/` to `/opt/pocket/services/example-charts`, builds the image, starts `example-charts-backend` on the supplier network, and waits for `/healthz`. Done 2026-09-13.
2. **Register the service** on Beta with the Service Manager (Services, Register). Done 2026-09-13 at 10,000 CU per relay.
3. **Re-stake the supplier for both services** with Supply service (server `example-host`), using [`deploy/supplier_stake_config.yaml`](deploy/supplier_stake_config.yaml) as the reference: the service list is replaced on every stake, so both ids are listed with the same endpoint.
4. **Relayer entry**: already present in [`servers/example-host/relayer-config.yaml`](../../servers/example-host/relayer-config.yaml) since the supplier migration on 2026-09-14. For another server, `bash deploy/add-to-relayer.sh` inserts [`deploy/relayer-service.yaml`](deploy/relayer-service.yaml) into that server's relayer config and recreates the relayer.
5. **Stake an application wallet for it.** An account can be staked as an application for exactly one service, so on the Wallets tab create `app-example-charts`, fund it from the owner wallet, and stake it on the Stake tab (that wallet is preselected under "Stake as"). Registration recorded on 2026-09-13 at 10,000 CU per relay; the manifest carries the transaction hash.
6. **Relay through the protocol** from the example host: export the app wallet's key (Wallets tab, Export key) into `POCKET_APP_PRIVATE_KEY` on the server, or put it in `deploy/app-key.hex` (mode 600, never committed), then after one session `bash deploy/relay-test.sh` sends GET and POST relays with pocket-ap. Without an exported key the script falls back to the throwaway `test-app` from the first service once `bash deploy/stake-test-app.sh` has re-pointed it at example-charts. Watch claims with the Skill's `query_state.py --claims`.
...
- Publish the OpenAPI document and the operator docs at the URLs the card names under `example.org`, then add the `sha256` to `specs[0]`. Both URLs are placeholders today.
