#!/usr/bin/env bash
# Beta TestNet only. Run ON the supplier host. Sends relays for example-charts through
# the protocol with pocket-ap, signed by an application staked for example-charts.
#
# Key source, in order:
...
#      stake-test-app.sh re-pointed it at example-charts.
# The application must be staked for example-charts and the supplier must be in its
# current session, so wait one session after staking.
set -euo pipefail
D="$(cd "$(dirname "$0")" && pwd)"
APP_HOME="${APP_HOME:-/opt/pocket/services/example-builder-test/app-home}"
