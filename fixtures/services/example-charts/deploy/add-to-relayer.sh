...
# for it. Adds the example-charts block (relayer-service.yaml) to the HA relayer's
...
#
#   bash /opt/pocket/services/example-charts/deploy/add-to-relayer.sh [supplier-stack-dir]
#
...
if grep -qE '^  example-charts:' "$CFG"; then
  echo "relayer config already lists example-charts; nothing to add"
else
...
open(cfg, "w").write("\n".join(lines))
print("inserted example-charts block into", cfg)
PY
fi
...
docker logs --tail 15 "$PROJECT-relayer" 2>&1 | grep -iE "example-charts|error|backend" || true
