#!/usr/bin/env python3
"""Minimal example-charts backend skeleton for Pocket Network.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/":
            # The RelayMiner's backend ping hits the backend root at startup and on
            # /ping; anything but a 2xx makes it report the backend unreachable.
            return self._json(200, {"service": SERVICE, "status": "ok"})
        return self._json(404, {"error": {"code": "not_found", "message": "unknown path"}})

    def do_HEAD(self):
        # Reachability probes may use HEAD; BaseHTTPRequestHandler answers 501 otherwise.
