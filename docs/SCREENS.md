...
**Execute (`executeStake`).** Form-changed guard (id, from, upokt). MainNet modal "Confirm MainNet stake" (type the service ID; dangerbox "This locks X POKT of real funds from <wallet> as an application stake for <id>. Unstaking takes an unbonding period.") or Beta `confirm`. Then log "Signing and broadcasting stake-application for '<id>' as '<wallet>' with X POKT on <net>", status busy, `run("tx-stake-app", {...})`, mempool log with tx link, `pollTx`, "Included in block N.", verify via `appRecordOf(address)` that the service id is present and `stake.amount >= upokt` (log ok/err "On chain: '<wallet>' staked X POKT for '<ids>'."), status ok "'<wallet>' is staked as an application for '<id>' on <net>."; the three failures are reported apart as in 3.10 (record unreadable, stake below what was submitted, not staked for the service), and the read goes through `readAfterTx()`; on success `recordManifest("app_stake_tx", txhash)`, `("app_wallet", from)`, `("app_address", address)`; then `refreshBalance(); loadHistory(); loadWallets(false, → onStakeFromChange(); renderServices())`.
...
**Unstake this application (Electron only, product owner, 2026-09-20).** `.panel#stkUnstakePanel` below the preflight results. Shown only when the chosen wallet has an application record. While one is already unbonding it is a `.warnbox` naming the last block it can relay in and, from the live `application_unbonding_period_sessions` and `num_blocks_per_session`, the block its stake comes back at; staking again before then cancels it. Otherwise a hint and `.btn.danger#btnUnstakeApp` "Unstake application" running `unstakeApp()`: `confirmTx` with a dangerbox naming the wallet and the service it is staked for, the stake, the live unbonding period in sessions and time, and that the wallet is kept (MainNet: type `UNSTAKE`; Beta: a modal button), then `tx-unstake-app`, `pollTx`, a `readAfterTx` re-read of the application record, and `refreshNetwork(); loadHistory(); refreshBalance(); loadWallets()`. The application wallet signs for itself and pays the gas; the owner wallet is not involved. The operation is deliberately absent from the MCP bridge.
...
**Execute (`executeSupply`).** Form-changed guard (server, operator, amount, and the JSON of currently ticked rows). MainNet modal (type the server name; dangerbox "This locks X POKT of real funds as a supplier stake for operator <op>, serving <ids>. Unstaking takes S sessions.") or Beta `confirm`. Log "Copying the stake config to <server> and signing with the operator key there, X POKT for <ids> on <net>", status busy "Waiting for the server (simulating gas, signing, broadcasting)", `run("remote-stake-supplier", {...no dry})`, mempool log, `pollTx`, "Included in block N.", then verify: re-read the supplier record; `services[]` is the active set and `service_config_history[]` entries with `deactivation_height == 0` are scheduled (`activation_height`); `okv` requires `stake.amount >= upokt` and every requested id either active or scheduled; log "On chain: supplier <op> staked X POKT; active for <ids>; <pending ids> scheduled, activates at block H; <missing ids> neither active nor scheduled."; status ok "Supplier staked on <net> for <ids>. <pending ids> is scheduled and activates at block H, the next session boundary."; on success `recordManifestFor(folder, ...)` for each served service with a local folder: `supplier_stake_tx`, `supplier_operator`, `supplier_url`, `deploy_host`, `deploy_path`. Then `refreshBalance(); refreshOperatorBalance(); loadHistory(); loadSupplierServices(); renderServices()`.

**Electron deviation (by design, 2026-09-20).** The read-back goes through `readAfterTx()`: the LCD cache is dropped first, because every cached answer is stale the moment a transaction lands, and an unusable answer is read once more after 3 s, because the node is at its least reliable in the seconds after one. The three ways verification can fail are then reported apart, where all three used to produce the same red sentence about the service list: the record could not be read (status `''`, neutral, "Staked in block N, but the supplier record could not be read back just now, so this is unconfirmed. Open Suppliers in a moment to check it." - the transaction succeeded and the app does not claim otherwise), the on-chain stake is below what was submitted, or a submitted service is neither active nor scheduled. A scheduled service has never been a failure: a stake does not add a service to the session already running, it appears in the config history with the next boundary as its activation height and joins `services[]` there. Section 3.9's application stake splits the same three ways.
...
**Probe list (`testProbes(id)`).** From the local card's `serving.healthcheck[]`: label "Identity probe" / "Readiness probe" / "Functional probe" by notes text, `"<label> <METHOD> <path>"`; each POST probe with a body adds "Bad input POST <path>" expecting a 4xx JSON error, sent with `BAD_INPUT_BODY`, a lone `{` that no JSON parser accepts. **Electron deviation (by design, 2026-09-20).** The HTA sent `{}` here, which assumes an empty object is always missing a required field. It is not: a service whose fields are all optional answers 200 to it, correctly, and the probe graded a sound service red. Input nothing can parse is the one bad input every JSON service agrees on, and is what the Skill's `lint_backend.py` probes with. Without a card: "Identity probe GET /v1/version" (`$.service` matches `^<id>$`) and "Readiness probe GET /healthz" (`$.status` matches `^ok$`).

**Probe retry (Electron only, product owner, 2026-09-20).** A relay that comes back with no HTTP status and a non-zero exit (`relayIncomplete()` in `src/core/probes.ts`) never reached the service, so it is not graded as the service answering badly: the screen logs "The relay did not complete. Sending it once more.", waits two seconds, and sends it again. The first relay into a session that has only just started is where this happens, arriving truncated as an `unexpected EOF` while the supplier's relayer catches up with a session it has just joined; a probe that then passes was never a fault of the service. Only the relay layer is retried. Any HTTP answer, including a 5xx, is graded as it stands. The ready line says so in advance when the session is at most one block old: "It started this block, so a first relay may have to be sent twice; the test does that itself."
...
- the address holds no application record at all (`isNoApplicationError()`, the node's "could not find app with address") -> `badge warn` "not ready" and "This wallet is not staked as an application for <id>, so the protocol has no session for it and a relay has nothing to sign. Stake one for <id> first, then test from the session after that." This is a state to act on rather than a failure to read, so it blocks the test like the others; none of the node's text reaches the screen.
- the query failed for any other reason -> `badge warn` "session unknown" and "Could not read the current session from the network — <detail>. The test will run and show whatever the protocol answers." `<detail>` is `shortLcdDetail()`: the status and the node's own `message`, capped at 160 characters, with the URL and the rest of the JSON body dropped, because an LCD failure otherwise arrives as a query string and a JSON blob and fills the panel.

The line itself is `.sessionline`: the badge and "Check again" on one row, the sentence under them at up to 90 characters a line, so neither wraps around the other. "Run test" is disabled while the state is waiting and while the first check is in flight; `runTest()` re-runs the check and refuses with the same sentence if the answer is still waiting, so a stale line cannot let a doomed test through. A `.btn.small` "Check again" re-asks at any time, and a waiting-for-the-boundary state re-asks itself every 20 s until it clears (a missing supplier does not, because that one needs the user to act). An answer is rendered only against the service and wallet it was asked about. Every height, the session length, and the block time are read live; nothing here is a stored chain value.
...
**Return to owner (Electron only, product owner, 2026-09-20).** A `.btn.small` "Return to owner" on every application wallet row, between Stake and Export key, opening `returnToOwnerDialog(name)`. The dialog shows the wallet's balance, says the amount suggested leaves about 1 POKT behind because the sending wallet pays the fee, and explains why emptying the wallet is the normal end of its life: an application wallet is named after its service and cannot be reused for another, and after an unbonding the stake returns to it rather than to the owner. `returnToOwner()` in `lib/flows.tsx` checks the balance covers the amount plus gas, confirms (MainNet: type `SEND`), runs `tx-return-to-owner`, polls, and refreshes the balance and activity. The wallet itself is kept; Remove is still the separate action.
...
**Electron deviation (by design, product owner, 2026-09-20).** `returnToOwner()` is the third transfer and the only one that does not start at the owner wallet (3.9). All three take an optional `progress` title, and the two launched from a dialog pass one. There is a single modal at a time, and `typedConfirm` closes it when the user confirms, so a flow started from a dialog loses that dialog and the status line it was writing to: on MainNet the window fell silent for the minute a transaction takes to reach a block. With a title, the flow opens a `progressModal()` (`src/renderer/src/lib/modal.tsx`) after the confirmation and narrates into it: the send, the broadcast with the first ten characters of the hash, then the block it landed in. It is locked while the work runs and ends with a Close button carrying the outcome, so the result stays on screen until the user dismisses it rather than vanishing on a timer. The caller's own status line is still updated, which is what the on-screen callers (Stake application, the supplier editor) use; they pass no title and behave as before.
...
**Electron deviation (by design, product owner, 2026-09-20).** Step 7 also folds the preflight away. The checklist and the plan are what the user reads before pressing the button and are in the way afterwards: the run's own narration began below a screenful of something already agreed to, and on a slow transaction nothing visible said anything was happening. `PreflightSection` (`src/renderer/src/components/ui.tsx`) wraps the `Checks` and the `PlanBlock` on Register, Stake application and the supplier editor; while `ran` is false it is the usual "Preflight" heading, and once execute starts it becomes a single green "Preflight passed" that opens again on a click. Running preflight again unfolds it. The status line and the log stay outside it, because they are the run.
