...
- `electron-builder` producing an NSIS installer and a portable zip. GitHub Actions on `windows-latest` builds both, publishes `SHA256SUMS`, and commits the Scoop manifest. Scoop is the only supported distribution channel: its download path carries no Mark-of-the-Web, so the unsigned build runs, whereas a browser download of an unsigned app is refused outright by current Windows 11 installs (product owner, from earlier Electron builds); the installer and zip on the releases page exist for Scoop and for checksums, not as a manual install path; the bucket is `bucket/pocket-service-manager.json` in this repo. Signing hook present and disabled until credentials exist (`docs/PACKAGING.md`).
...
- **Update the compat file.** When the app gains or loses a capability, update `reference/mcp/src/compat.json` here so assistants know what the installed version can do. The `sync-compat` workflow mirrors it into the service-builder repository's `mcp/src/compat.json` on each `v*` tag, and that push redeploys the public MCP; do not hand-copy it. App version strings are `electron-<semver>`.
