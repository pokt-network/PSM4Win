...
- No Python and no PowerShell at runtime. The HTA depended on both; the Electron app must not. The one exception is the importer's one-time DPAPI unseal (`docs/MIGRATION.md`), which spawns Windows PowerShell once because DPAPI has no Node binding worth adding for a single call.
...
| `reference/` | Read-only material from the service-builder repository: `hta-app/` (the HTA, complete), `skill/` (the Claude Skill: card schema, templates, references, scripts), `mcp/` (the Cloudflare Worker with JavaScript ports of the Skill scripts and `compat.json`), `docs-pages/` (the three docs.pocket.network pages), `research/` (the deployment review whose section 5 lists every design constraint), `servers/cherry/` (the reference copy of a provisioned supplier host). Never edit these here; they are snapshots. |
| `fixtures/services/` | Two real service folders (`pretty-charts`, live on Beta and MainNet; `jinx-service-builder-test`, the Beta test service) without `node_modules`. Use them as test data for folder discovery, card validation, and Deploy. They contain no keys. |
...
The pristine handoff package lives in `pocket-service-manager-electron/`, which is gitignored and never published. The copies in this root are the same files with host and wallet details removed: the self-test server override in `reference/hta-app/app.js`, the server-entry paragraph in `reference/servers/cherry/README.md`, and the owner, operator, and application addresses in `fixtures/` now hold documentation placeholders (`203.0.113.10`, `REPLACE-user`, `id_supplier`, and `pokt1q...` addresses that belong to nobody). Real values are in the ignored package on the product owner's PC only. Never copy them into this tree.
